...
public class GameObject
{
	...
	FloatBuffer matBuffModel = BufferUtils.createFloatBuffer(4 * 4);
	FloatBuffer matBuffMV = BufferUtils.createFloatBuffer(4 * 4);
	FloatBuffer matBuffMVP = BufferUtils.createFloatBuffer(4 * 4);
    ...
    public void draw(GameShader shader, GameCamera camera, boolean deep)
    {
        ...
        if(shader.uni_MV >= 0)
        {
            Mat4 mv = matrix.multiply(camera.matView);
            mv.store(matBuffMV);
            matBuffMV.flip();
        ...
        if(shader.uni_MVP >= 0)
        {
            Mat4 mvp = matrix.multiply(camera.matVP);
